...
def main_app():
    ...
    app.configure(bg="#99ff99")

    # Label warna biru
    label = tk.Label(app, text="Selamat Datang !", bg="#990000", fg="white", font=("Arial", 14), pady=10)
    label.pack(pady=10)

    # Frame oranye
    frame = tk.Frame(app, bg="#0000ff", width=300, height=100)
    frame.pack(pady=20)
    tk.Label(frame, text="Frame", bg="#0000ff", fg="white", font=("Arial", 12)).pack(pady=10)

    # Button hijau
    def on_hover(e): button.config(bg="#43a047")
    def off_hover(e): button.config(bg="#4caf50")

    button = tk.Button(app, text="Keluar", bg="#ff0099", fg="white", font=("Arial", 12), command=app.destroy)
    button.pack(pady=10)
...
class AuthApp:
    ...
    def setup_widgets(self):
        ...
        switch_text = "Sudah punya akun? Login" if self.is_registering else "Register"
